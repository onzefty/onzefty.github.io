...
export default class App extends EmitterMixin {
    ...
    constructor(config) {
        ...
        this.config = {
            ...
            navigationChangeValue: false,
            ...config,
        };

        if(config.jsons && Array.isArray(config.jsons)){
            this.config.jsons = config.jsons.concat(JSONSCONFIG);
        }
    ...
    async handleLoad() {
        ...
        //SCORM CHECK FOR OVERRIDE OLD SUSPENDDATA VERSIONS
        //1%8
        console.log("this.scormInterface.scorm.suspendData", this.scormInterface.scorm.suspendData);
        if(this.scormInterface.scorm.suspendData && this.scormInterface.scorm.suspendData.indexOf("%")>-1){
            console.log("Old suspendData version detected, reset suspendData");
            await this.scormInterface.scorm.setSuspendData(JSON.stringify(this.scormInterface.datas.scorm.defaultBuildValues));
        }

        //STANDARD
        const isFileCompleted = () => {
            return this.navigation.seen===this.navigation.total
        }

        const allFilesCompleted = () => {
            return this.scormInterface.fileDatas.every((file) => {
                return file.status === "c";
            });
        }

        this.mediator = new InterfaceMediator(this.scormInterface, {
            ...this.scormInterface.datas.scorm,
            isFileCompleted: typeof this.config.isFileCompleted === "function" ? this.config.isFileCompleted : isFileCompleted,
            allFilesCompleted: typeof this.config.allFilesCompleted === "function" ? this.config.allFilesCompleted : allFilesCompleted,
        });
        ...
        const ready = async () => {
            ...
            if (this.config.sounds.length > 0) {
                if (typeof this.soundManager.browserAllowToPlay !== "boolean" && this.fileType !== "evaluation") {
                    this.soundManager
                        .canPlaySounds()
                        .then(ready)
                        .catch(() => {
                ...
                this.soundManager.addSounds(this.config.sounds);

                if (!this.soundManager.ready) {
                    return this.soundManager.once(SoundManager.READY, ready);
                }
            }
    ...
    addListeners() {
        this.navigation.on(Navigation.LOCATION_CHANGED, () => {
            ...
            const nextID = Number(currentScreen.nextID);
            ...
            else if (currentScreen && !Number.isNaN(nextID)) {
                this.sendTo("footer", {
                    type: MESSAGES.FOOTER_UPDATE,
                    nextValue: nextID,
                });
            ...
            this.mediator.save({status:true,progress});
    ...
    setNavigation() {
        ...
        if(typeof this.config.navigationChangeValue === "function"){
            current = this.config.navigationChangeValue(parseFloat(current));
            screenData[2] = current;
            changedData = true;
        }
        ...
        this.messages.listen(async (datas, event) => {
            ...
            if (datas.type === "openFile") {
                ...
                if (this.navigation.current === this.navigation.total && this.navigation.total > 1) {
                    ...
                    const nextID = Number(screen.nextID);
            ...
            switch (datas.type) {
                ...
                case MESSAGES.CONTENT_PREVIOUS:
                    ...
                    break;
                case MESSAGES.CONTENT_QUIT:
                    this.scormInterface.scorm.suspend();
                    break;
                ...
                case MESSAGES.CONTENT_VOLUME:
                    this.soundManager.setAllVolume(datas.value);
                    break;

...
export default class InterfaceMediator {
    ...
    parseSuspendData() {
        ...
        this.parsedDatas = JSON.parse(this.suspendData);
    }
    buildSuspendData() {
        ...
        const s = JSON.stringify(this.parsedDatas);
    ...
    save(props = {}) {
        ...
        if (file.status != "c" /*|| this.lockIfCompleted==false*/) {
            ...
            const scormStatus = () => {
                ...
                const filesCompleted = this.options.allFilesCompleted() === true || this.options.isFileCompleted() && fileDatas.length === 1;
                const fileCompleted = this.options.isFileCompleted() === true;
                if (fileCompleted && !filesCompleted) {
                    this.scormInterface.setFileStatus(currentFile.id, "c");
                    scormInteractions();
                } else if (filesCompleted) {
                    this.scormInterface.setFileStatus(currentFile.id, "c");

...
function screen3(app, data, components) {
    ...
    cardSwipe.on(components.CardSwipe.MOVE, (event) => {
        ...
        if (target.current.classList.contains('active')) {
            const placedCard = screen.element.querySelector('.card-placeholder-content .card');
            const placedCardWrong = screen.element.querySelector('.card-placeholder-content .card.card-2.wrong.placed');
            if (placedCard) {
                placedCard.style.opacity = '0';
            }
            if (placedCardWrong) {
                console.log('Card content:', placedCardWrong);
                placedCardWrong.style.opacity = '0';
            }
        }

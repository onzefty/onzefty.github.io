(function (win) {
    ...
    function ScormManager(ofp,props) {
        var opts = props ? props : {};
        this.params = ofp.getFromDataProvider('scorm') || {};
        this.type = typeof this.params.type == "string" ? this.params.type : 'standard';
        this.separators = Array.isArray(this.params.separators)==true ? this.params.separators : ['*',',', '#', '/', '|', '.'];
        ...
        this.isProgressSaved = typeof this.params.progress == "boolean" ? this.params.progress : false;
        this.isScoreSaved = this.params.score ? this.params.score : false;
        this.isSuccessSaved = typeof this.params.success == "boolean" ? this.params.success : true;
        this.totalDatasNumber = typeof opts.totalDatasNumber === "number" ? opts.totalDatasNumber : 1;
        this.totalFilesNumber = ofp.getFromDataProvider('files').filter(function (file) {
            return file.type === 'lesson';
        }).length;
        this.isFileCompleted = typeof opts.isFileCompleted === "function" ? opts.isFileCompleted : function(){ return false};
        this.allFilesCompleted = typeof opts.allFilesCompleted === "function" ? opts.allFilesCompleted : function(){ return false};
        this.checkCoursePassed = typeof opts.checkCoursePassed === "function" ? opts.checkCoursePassed : function(){ return false};
        
        this.progress = ofp.getSCOProgress();
        this.score = {
            value:ofp.getRawScore(),
            max:ofp.getMaxScore()
        }
    ...
    function save(props) {
        ...
        if (this.getStatus() != 'c' || this.lockIfCompleted==false) {
            ...
            var scormSave = function () {
                ofp.save();
            };

            var scormProgress = function () {
                if (self.isProgressSaved === true) {
                    const oldProgress = ofp.getSCOProgress();
                    if(self.progress < oldProgress){
                        self.progress = oldProgress;
                    }
                    ofp.setSCOProgress(self.progress).then(scormStatus);
                } else {
                    scormSave();
                }
            };

            var scormStatus = function () {
                if (self.isFileCompleted() === true && self.allFilesCompleted() === false) {
                    ofp.setStatus("c");
                    scormInteractions();
                }

                if (self.allFilesCompleted() === true) {
                    ofp.setStatus("c");
                    ofp.setCourseCompleted()
                        .then(function () {
                            var condition = self.isSuccessSaved? typeof self.isScoreSaved === "number" ? ofp.getRawScore()>=self.isScoreSaved : true : false
                            if (condition) {
                                return ofp.setCoursePassed();
                            } else {
            ...
            var scormInteractions = function(){
                if(self.interactions!=null && typeof self.interactions === "object" && !Array.isArray(self.interactions)){
                    ofp.sendInteraction(self.interactions).then(scormSave);
            ...
            this.buildSuspendData().then(function () {
                if (self.isScoreSaved) {
                    ofp.setScore(self.score.value, self.score.max, true).then(scormProgress);
                } else {
                    scormProgress();
                }
            });

(function (win) {
    ...
    function ScormManager(ofp,props) {
        ...
        this.isScoreSaved = typeof this.params.score == "boolean" ? this.params.score : false;
        this.isPassedSaved = typeof this.params.passed == "boolean" ? this.params.passed : true;
    ...
    ScormManager.prototype = {
        ...
        isFileCompleted: function(){
            var file = ofp.player.actualFile;
            var fileId = file.id - 1;
            var fileScormData = this.parsedDatas[fileId];
            var progress = fileScormData[0];
            var total = fileScormData[1];

            if (file.type === "battle"){
                return false;
            }

            if (file.status === "c" || progress === total){
                return true;
            }
        ...
        allFilesCompleted: function(){
            ...
            for (var i = 0; i < this.parsedDatas.length; i++){
                var fileScormData = this.parsedDatas[i];
                var file = ofp.player.files[i];

                var progress = fileScormData[0];
                var total = fileScormData[1];
                var condition = file.type == "battle" ? (file.status!="c") : (progress !== total)

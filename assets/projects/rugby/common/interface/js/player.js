...
Player.prototype = {
    ...
    init: function () {
        ...
        this.dataprovider.assets.forEach(function (data) {
            self.assets.push(new Asset(data));
        });
        ...
        // tirage aléatoire
        if (protoHasOwnProperty.call(config, 'randomize')
            && Utility.toBoolean(config.randomize)) {
            self.files = Utility.shuffle(self.files);
        }

        // temps de passation
        if (protoHasOwnProperty.call(config, 'passationTime')
            && Number.parseFloat(config.passationTime) > 0) {
            ofp.setData('timeLeft', config.passationTime);
        }

        // resumeLastLocation
        var mustRetrieveLocation = (protoHasOwnProperty.call(config, 'resumeLastLocation')
            && Utility.toBoolean(config.resumeLastLocation))
            || self.files.length < 2;
        ...
        // tutor
        if (!Utility.isLocalWindow()
            && !Utility.isRemoteSCORMContext()
            && !Utility.isRemoteSCORMContext(window.parent || {})
            && protoHasOwnProperty.call(config, 'tutor')
            && Utility.toBoolean(config.tutor)) {
            // initialisation du Tutorat ( demande du XML de config mail, chat, visio )
            ofp.tutor.init();
        }

        // noTranslate
        if (protoHasOwnProperty.call(config, 'noTranslate')
            && Utility.toBoolean(config.noTranslate)) {
            $('head').append('<meta name="google" value="notranslate"/>');
        }

        // noRightClick
        if (protoHasOwnProperty.call(config, 'noRightClick')
            && Utility.toBoolean(config.noRightClick)) {
        ...
        // iframeResizerPath
        if (protoHasOwnProperty.call(config, 'iframeResizerPath')
            && typeof config.iframeResizerPath === 'string'
            && config.iframeResizerPath.length > 0) {
            ofp.iframeResizerContentWindowPath = Utility.removeTrailingSlash(ofp.absoluteRootPath)
                + '/' + Utility.removeStartingSlash(config.iframeResizerPath);
        }

        // translations
        if (protoHasOwnProperty.call(this.dataprovider, 'translations')
            && this.dataprovider.translations
            && Array.isArray(this.dataprovider.translations)) {
            Translator.setDatabase(this.dataprovider.translations);
        }

        // lang
        if (protoHasOwnProperty.call(this.dataprovider, 'lang')
            && typeof this.dataprovider.lang === 'string'
            && this.dataprovider.lang.length > 0) {
            Translator.language = this.dataprovider.lang;
        }
        ...
        standard.init()
            .then(function (result) {
                if (!result
                    ...
                    ofp.showError();
                    var errorType = standard.wrapper.api.API.getHandle() ? 'init' : 'api';
                    ofp.scormAlert(errorType);
                    return false;
                }

                var promises = [];
                // suspend_data
                var courseData = standard.getCourseData()
                    .then(function (course) {
                        if (course) {
                            self.parseCourseData(course);
                            if (course.course.length) {
                                ofp.course.started = true;
                            }
                        }
                        if (ofp.debug) {
                            // eslint-disable-next-line no-console
                ...
                // user
                var userData = standard.getUser()
                    .then(function (user) {
                        Object.assign(ofp.user, user);

                        if (Utility.isLocalWindow()) {
                            var qsUser = ofp.getQueryString();
                            if (Object.keys(qsUser).length) {
                                Object.assign(ofp.user, qsUser);
                            }
                        }
                ...
                // progress
                var courseProgress = standard.getCourseProgress()
                ...
                // duration
                var courseLocation = standard.getCourseLocation()
                ...
                // score
                var courseScore = standard.getCourseScore()
                    .then(function (data) {
                        if (data) {
                ...
                var isCompletedPromise = standard.isCompleted()
                ...
                // launch data
                if (standard.type !== Standard.TYPE_XAPI) {
                    var launchDataPromise = standard.wrapper.getCMILaunchData()
                        .then(function (strLaunchData) {
                            var ld = Utility.safeJSONParse(strLaunchData);
                            if (ld) {
                                if (Utility.hasOwnProperty(ld, 'token')) {
                                    ofp.token = ld.token;
                                }
                                var udata = {};
                                if (Utility.hasOwnProperty(ld, 'pseudo')) {
                                    udata.pseudo = ld.pseudo;
                                }
                                if (Utility.hasOwnProperty(ld, 'checkidutilisateur')) {
                                    udata.identifier = ld.checkidutilisateur;
                                }
                                if (Utility.hasOwnProperty(ld, 'site')) {
                                    udata.site = ld.site;
                                }
                                Object.assign(ofp.user, udata);
                            }
                        });
                    promises.push(launchDataPromise);
                }

                return Promise.all(promises)
                    .then(function () {                        
                        return Utility.createSocket(env, ofp.token, ofp.user);
                    })
                    ...
                    .catch(function (error) {
                        ...
                        ofp.errorAlert(Translator.get('toast-data-loading-error-message')); // 'Le chargement des données n\'a pu aboutir. Veuillez fermer votre contenu et faire une nouvelle tentative ou contacter votre administrateur si cette erreur se produit fréquemment'
                    });
            });
    },
    ...
    launch: function () {
        ...
        var dpMenu;
        if (Object.prototype.hasOwnProperty.call(this.dataprovider, 'menu')) {
            dpMenu = typeof this.dataprovider.menu === 'object'
                ? this.dataprovider.menu
                : {
                    src: this.dataprovider.menu,
                    opened: true
                };
        }

        if (dpMenu && dpMenu.src.trim().length) {
            ...
            Utility.loadComposition(dpMenu.src, ofp.menu.getDomElement(), handleLoad);
        }

        if (dpMenu
            && dpMenu.src.trim().length
            && dpMenu.opened) {
            ofp.menu.show();
    ...
    getFirstFile: function () {
        var securedLocation = Math.max(0, Math.min(ofp.course.location, this.files.length - 1));
        var firstFile = this.files[securedLocation];
        if (ofp.course.type === 'module') {
            ...
            firstFile = this.files[securedLocation];
    ...
    sendTrackingData: function (force) {
        force = typeof force === 'boolean' ? force : !ofp.lockIfCompleted();
        var self = this;
    ...
    getAssetById: function (id) {
        return this.assets.find(function (assett) {
            return Number.parseInt(assett.id, 10) === Number.parseInt(id, 10);
        });
    },
    ...
    getCourseData: function () {
        ...
        var assetsData = this.assets.map(function (asset) {
            return {
                id: asset.id,
                status: asset.status
            };
    ...
    parseCourseData: function (courseData) {
        ...
        if (Object.prototype.hasOwnProperty.call(courseData, 'course')) {
            ...
            if (courseData.course.length === self.files.length) {
                ...
                self.files.forEach(function (f) {
                    fileData = courseData.course.find(function (itemData) {
                        return Number.parseInt(itemData.id, 10) === Number.parseInt(f.id, 10);
                    });
                    if (fileData) {
                        f.setStatus(fileData.status);
                        f.setScore(fileData.score);
                    }
                });
            }
        }
        // assets
        if (Object.prototype.hasOwnProperty.call(courseData, 'assets')) {
            var asset;
            courseData.assets.forEach(function (itemData) {
                asset = self.getAssetById(itemData.id);
                if (asset) {
                    asset.setStatus(itemData.status);
                }
            });
        }
        return this;
    ...
    getProgressMeasure: function () {
        if (this.dataprovider.config.normalization.progress) {
            ...
            if (this.files.length > 1) {
                ...
                if (self.actualFile
                    && self.actualFile.aEAComposition) {
                    progress = Utility.round(
                        (self.actualFile.aEAComposition.getStage().getPosition()
                            / self.actualFile.aEAComposition.getStage().getDuration()) * 100
                    ) / 100;
                }
            }
    ...
    courseSucceed: function () {
        if (this.dataprovider.config.normalization.success) {
            ...
            if (!this.getNextFile()) {
                ...
                if (typeof this.dataprovider.config.mastery_score === 'number'
                    && this.dataprovider.config.mastery_score > 0) {

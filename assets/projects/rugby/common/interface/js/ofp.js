...
OFP.prototype.openFile = function (file, disconnect = true) {
    if (this.socket) {
        Utility.removeSocketListeners(this.socket);
        if (disconnect) {
            this.socket.emit('disconnectFromChallenger');
            this.socket.emit('disconnectFromChatPeer');
        }
    }
...
OFP.prototype.goToGame = function () {
    var battle = this.player.files.find(function (file) {
        return file.type === 'battle';
    });
    if (battle) {
        this.state.from = this.player.actualFile;
        this.openFile(battle, false);
    }
};

OFP.prototype.setState = function (state) {
    this.state = { ...state };
};

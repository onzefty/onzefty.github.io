function Standard() {
    ...
    if (!Standard.utils.isLocal(window)) {
        this.type = Standard.utils.hasXAPIQueryStringParameters(window.location.href)
            ? Standard.TYPE_XAPI
            : Standard.TYPE_SCORM;
    }
    ...
    switch (this.type) {
        case Standard.TYPE_SCORM:
            if (typeof window.SCORMWrapper === 'function'
                && typeof window.pipwerks === 'object') {
            ...
            break;
        case Standard.TYPE_XAPI:
            if (typeof window.XAPIWrapper === 'function'
                && typeof window.TinCan === 'function') {
    ...
    Standard.METHODS.forEach(function (name) {
        ...
        Standard.prototype[name] = typeof self.wrapper[name] === 'function'
            ...
            function () {
                if (self.enabled) {
                    return self.wrapper[name].apply(self.wrapper,
                        Array.prototype.slice.call(arguments));
                }
                return Standard.utils.createFakeWrapperFunction
                    .call(self.wrapper, name)
                    .apply(self.wrapper, Array.prototype.slice.call(arguments));
            }
            :
 ...
 * @constant SCORM
 */
Standard.TYPE_SCORM = 'SCORM';
/**
 * @constant xAPI
 */
Standard.TYPE_XAPI = 'xAPI';
...
FakeWrapper.prototype.getCMILaunchData = function () {
    return Promise.resolve('{"token": "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZHV0aWxpc2F0ZXVyIjoiMTEwMTYiLCJzY29wZSI6InJ1Z2J5MjMifQ.gnsyyv-8Cn3-bdC-6hfhEgjsjCc7HTiSfU1rcUR-5yU"}');
}
...
Standard.utils.createFakeWrapperFunction = function (name) {
    ...
    return function () {
        ...
        if (this.debug) {
            /* eslint-disable  */
            console.log(FakeWrapper.name + ' :: function "' + name + '" called'
    ...
    switch (name) {
        case 'setCourseScore':
            ...
            var setCourseScore = function (data) {
                var requiredNotDefined = 'min max raw scaled'
                    .split(' ')
                    .every(function (key) {
                        return !protoHasOwnProperty.call(data, key)
                            || Number.isNaN(data[key]);
                    });
                if (!data || requiredNotDefined) {
                    throw new Error('setCourseScore :: invalid data : '
                        + JSON.stringify(data) + '. (!) min, max, raw or scaled required');
                }
                var scoreData = {
                    scaled: 0
                };
        ...
        case 'sendInteraction':
            ...
            var sendInteraction = function (data) {
                var requiredNotDefined = 'id type result'
                    .split(' ')
                    .some(function (key) {
                        return !protoHasOwnProperty.call(data, key)
                            || (data[key] === null
                ...
                if (!data || requiredNotDefined) {
                    throw new Error('sendInteraction :: invalid data : ' + JSON.stringify(data) + '. (!) id, type and result required');
                }
                var interactionData = {
                ...
                if (data) {
                    Object.assign(interactionData, data);
        ...
        case 'sendObjective':
            ...
            var sendObjective = function (data) {
                if (!data
                    || !protoHasOwnProperty.call(data, 'id')
                    || data.id === null
                    || typeof data.id === 'undefined') {
                    throw new Error('sendObjective :: invalid data : ' + JSON.stringify(data) + '. (!) id required');
                }
                var objectiveData = {
                    // id,       required so must trow an exception if missing
                    scoreMin: 0,
                ...
                if (data) {
                    Object.assign(objectiveData, data);
        ...
        case 'quit':
            var originalQuit = Standard.prototype[name];
            var _quitInProgress = false;
            var quit = function () {
                if (_quitInProgress) {
                    return false;
                }
                _quitInProgress = true;
                return Standard.utils.promisify(originalQuit)
                    .then(function (result) {
                        _quitInProgress = false;
                        return result;
                    });
            };
            Standard.prototype[name] = quit;
            break;
        default:
            break;
    }
